...
import in.learntech.ezae.DataStoreMgr.UserDataStore;
...
public class UserMgr {
    ...
    public void saveUserFromResponse(JSONObject response)throws Exception{
        ...
        String vendorLogo = userJson.getString("vendorLogo");
        ...
        user.setVendorImage(vendorLogo);
    ...
    public String getLoggedInUserVendorImageUrl(){
        int userSeq  = mPreferencesUtil.getLoggedInUserSeq();
        User user = userDataStore.getUserByUserSeq(userSeq);
        String vendorImage = StringConstants.USER_DUMMY_IMAGE_NAME;
        if(user.getVendorImage() != null && !user.getVendorImage().equals("") && !user.getVendorImage().equals("null")){
            vendorImage = user.getVendorImage();
        }
        return StringConstants.IMAGE_URL + "AdminImages/vendorLogo/"+vendorImage;
    }
