...
public class MyTrainings_LearningPlansFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener, IServiceHandler{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View drawerLayout = inflater.inflate(in.learntech.ezae.R.layout.my_training_learningplans_fragment, container, false);
        mPrentLayout = (LinearLayout) drawerLayout.findViewById(in.learntech.ezae.R.id.layout_lp);
        swipeLayout = (SwipeRefreshLayout) drawerLayout.findViewById(in.learntech.ezae.R.id.swipe_container);
            ...
            if(success){
                ...
                for (int i=0; i < count; i++) {
                    ...
                    LinearLayout lpInternalLayout = (LinearLayout)
                            mInflater.inflate(in.learntech.ezae.R.layout.my_training_learningplan_header, mContainer, false);
                    ProgressWheel progressWheel = (ProgressWheel)lpInternalLayout.findViewById(in.learntech.ezae.R.id.progressBar_lp);
                    progressWheel.setStepCountText(String.valueOf(progress)+"%");
                    progressWheel.setPercentage(progress*4);

                    TextView  textView_lpName = (TextView)lpInternalLayout.findViewById(in.learntech.ezae.R.id.textView_learningPlanName);
                    textView_lpName.setText(title);

                    JSONArray modulesJsonArr = lpJson.getJSONArray("modules");
                    int moduleCount = modulesJsonArr.length();
                    TextView  textView_totalModules = (TextView)lpInternalLayout.findViewById(in.learntech.ezae.R.id.textView_lpTotalModules);
